...
export default function PantallaCodigoContraseñaDisp({ navigation }) {
    ...
    const confirmChangeDevicePassword = async (codigo) => {
        ...
        const peticion = await axios.post("https://proyecto-backend-movil-production.up.railway.app/app_movil_sensor/api/device/confirm-change-device-password", notification, headers)
            .then(async res => {
    ...
    const enviarCodigo = () => {
        ...
        setLoading(true);
        setTimeout(async () => {
            confirmChangeDevicePassword(formData.codigo);
    ...
    return (
        <View style={Styles.container}>
            <View style={Styles.registerContainer}>
                ...
                >
                    Ingrese el codigo que enviamos a: <Text style={{ fontWeight: "bold" }}>{emailUsuario}</Text>
