...
export default function CambiarRedWifi({ navigation }) {
    ...
    const cambiarRed = async (id, password) => {
        ...
        const peticion = await axios.post("https://proyecto-backend-movil-production.up.railway.app/app_movil_sensor/api/device/change-device-wifi", notification, headers)
            .then(async res => {
                navigation.goBack();
            })
            .catch(error =>
