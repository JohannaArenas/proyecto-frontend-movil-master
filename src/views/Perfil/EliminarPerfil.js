...
export default function EliminarPerfil({ navigation }) {
  ...
  const validateData = () => {
    ...
    /*if (formData.password !== "Password456" & formData.password !== "") {   //cambiar validacion
  ...
  const deleteUser = async (contraseña) => {
    ...
    const peticion = await axios
      .post(
        "https://proyecto-backend-movil-production.up.railway.app/app_movil_sensor/api/user/delete-user",
        notification,
        headers
      )
      .then(async (res) => {
      ...
      .catch(
        (error) => setErrorPassword("Contraseña incorrecta."),
        handleOnPress()
        // setLoading(false)
      );
  };
    ...
    <View style={Styles.container}>
      <View style={Styles.formContainer}>
        <View style={Styles.input}>
          ...
          >
            <FontAwesomeIcon icon={data.secureTextEntry ? faEyeSlash : faEye} />
          </TouchableOpacity>
        </View>
        {errorPassword !== null ? (
