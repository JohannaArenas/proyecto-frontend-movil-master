...
export default function PantallaCodigoContraseñaUsuario({ navigation }) {
  ...
  const confirmChangeUserPassword = async (codigo) => {
    ...
    const peticion = await axios
      .post(
        "https://proyecto-backend-movil-production.up.railway.app/app_movil_sensor/api/user/confirm-password",
        notification,
        headers
      )
      .then(async (res) => {
      ...
      .catch((error) => console.log(error), setErrorCodigo("error"));
  };
  ...
  return (
    <View style={Styles.container}>
      <View style={Styles.registerContainer}>
        ...
        >
          Ingrese el codigo que enviamos a:{" "}
          <Text style={{ fontWeight: "bold" }}>{mailUsuario}</Text>
