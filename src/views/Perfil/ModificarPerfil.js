...
export default function ModificarPerfil({ navigation }) {
  const [formData, setFormData] = React.useState(
    defaultFormValues(nombre, apellido, mail, nacimiento, nacionalidad)
  );
  ...
  const getUserData = async () => {
    ...
    const peticion = await axios
      .get(
        "https://proyecto-backend-movil-production.up.railway.app/app_movil_sensor/api/user",
        headers
      )
      .then(async (res) => {
  ...
  const changeUserData = async (
    ...
    const peticion = await axios
      .put(
        "http://192.168.0.133:8081/app_movil_sensor/api/user/modify-data",
        notification,
        headers
      )
      .then(async (res) => {
            ...
            >
              <View style={Styles.modalView}>
                <Text style={{ textAlign: "center", marginTop: 10 }}>
                  ¿Seguro que quiere modificar su perfil?
                </Text>
                    ...
                    <Boton
                      ...
                      onPress={() =>
                        changeUserData(
                          ...
                          date /*== new Date() ? date : formData.nacimiento*/,
                          formData.mail
                        )
                      }
                      type="aceptar"
