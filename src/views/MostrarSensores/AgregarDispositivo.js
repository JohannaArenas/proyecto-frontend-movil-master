...
export default function AgregarDispositivo({ navigation }) {
  ...
  const addDisp = async (code, password) => {
    ...
    const peticion = await axios
      .put(
        "https://proyecto-backend-movil-production.up.railway.app/app_movil_sensor/api/device/link-user",
        notification,
        headers
      )
      .then(async (res) => {
      ...
      .catch((error) => console.log(error), setErrorCode("Accceso denegado."));
  };
