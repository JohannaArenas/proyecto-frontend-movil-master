...
export default function PantallaCodigo({ navigation }) {
  ...
  const enviarToken = async (codigo) => {
    ...
    const peticion = await axios
      .post(
        "https://proyecto-backend-movil-production.up.railway.app/app_movil_sensor/api/auth/confirm",
        notification,
        headers
      )
      .then((res) => {
      ...
      .catch((error) => {
        setLoading(false), setErrorCodigo("El codigo es incorrecto.");
      });
  };
    ...
    <View style={Styles.container}>
      <View style={Styles.registerContainer}>
        ...
        >
          ...
          <Text style={{ fontWeight: "bold" }}>{route.params.mail}</Text>
        </TextoDripsy>
